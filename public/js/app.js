(function ($) {
    ...
    if (typeof window.lgModules === 'undefined' && typeof lgZoom !== 'undefined' && typeof lgThumbnail !== 'undefined') {
    ...
    const handleAjaxError = (xhr, $feedback) => {
        ...
        const errors = response.errors ? Object.values(response.errors).flat().join('<br>') : '';
        $feedback.removeClass('d-none alert-success').addClass('alert alert-danger').html(`${message}${errors ? '<br>' + errors : ''}`);
    };

    const handleAjaxSuccess = (response, $feedback, successMessage) => {
        $feedback.removeClass('d-none alert-danger').addClass('alert alert-success').text(response.message || successMessage);
    };
    ...
    const initContactForm = () => {
        ...
        $form.on('submit', function (e) {
            ...
            $.post($form.attr('action'), formData)
                .done((response) => {
                    handleAjaxSuccess(response, $feedback, 'Message sent. We will respond shortly.');
                    $form.trigger('reset');
                })
    ...
    const initLoginForm = () => {
        ...
        $form.on('submit', function (e) {
            ...
            $.post($form.attr('action'), formData)
                .done((response) => {
                    handleAjaxSuccess(response, $feedback, 'Login successful. Redirecting...');
                    setTimeout(() => window.location.href = response.redirect || '/admin', 800);
                })
                .fail((xhr) => handleAjaxError(xhr, $feedback));
        });
    };
    ...
    const initLightGallery = () => {
        const galleries = [document.getElementById('project-gallery'), document.getElementById('home-portfolio')];
    ...
    const initServicesAdmin = () => {
        ...
        $form.on('submit', function (e) {
            ...
            $.ajax({
                ...
                alert(xhr.responseJSON?.message || 'Unable to save service.');
            });
        });
    ...
    const initProjectsAdmin = () => {
                ...
                const card = $(`
                    <div class="col-md-6 gallery-card" data-id="${image.id}" draggable="true">
                        <div class="ratio ratio-16x9 rounded-4 overflow-hidden shadow-sm">
                            <img src="/storage/${image.file_path}" class="w-100 h-100 object-fit-cover" alt="${image.caption || ''}">
                        </div>
                        <button class="btn btn-sm btn-outline-danger mt-2" data-action="remove-image" data-id="${image.id}" data-project="${projectId}">Remove</button>
                    </div>
                `);
        ...
        const persistGalleryOrder = (projectId) => {
            if (!projectId) return;
            const order = $galleryGrid.find('.gallery-card').map((index, el) => $(el).data('id')).get();
            $.post(endpoints.reorderImages(projectId), { order })
                .fail(() => alert('Unable to save gallery order.'));
        };
        ...
        $form.on('submit', function (e) {
            ...
            $.ajax({ url, method, data: $form.serialize() })
                .done(() => { $modal.modal('hide'); refreshTable(); })
                .fail((xhr) => alert(xhr.responseJSON?.message || 'Unable to save project.'));
        });
        ...
        $uploadForm.on('submit', function (e) {
            ...
            $.ajax({
                ...
                $.get(endpoints.show(projectId)).done(({ data }) => renderGallery(data.images || [], projectId));
                $uploadForm.trigger('reset');
                refreshTable();
            }).fail(() => alert('Upload failed. Please check the image size.'));
        });
        ...
        $galleryGrid.on('click', '[data-action="remove-image"]', function () {
            ...
            $.ajax({ url: endpoints.deleteImage(projectId, imageId), method: 'DELETE' })
                .done(() => {
                    $.get(endpoints.show(projectId)).done(({ data }) => renderGallery(data.images || [], projectId));
                    refreshTable();
    ...
    const initTeamAdmin = () => {
        ...
        $form.on('submit', function (e) {
            ...
            $.ajax({
                ...
                dataTable.ajax.reload(null, false);
                $modal.modal('hide');
            }).fail((xhr) => alert(xhr.responseJSON?.message || 'Unable to save team member.'));
        });
    ...
    const initContactAdmin = () => {
        ...
        $modal.on('click', '[data-action="delete"]', function () {
            ...
            if (!confirm('Delete this message?')) return;
            $.ajax({ url: endpoints.delete(id), method: 'DELETE' })
                .done(() => { dataTable.ajax.reload(null, false); $modal.modal('hide'); })
                .fail(() => alert('Failed to delete message.'));
        });
    };
